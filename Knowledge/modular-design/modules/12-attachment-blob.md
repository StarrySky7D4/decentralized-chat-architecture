...
客户端侧负责附件导入、流式加密、分块、内容寻址 manifest、上传/下载、完整性校验和解密导出；服务侧只保存 opaque ciphertext chunks。
...
100 MiB 文件全流程不整文件入内存；任一 chunk 篡改均不会产生明文输出成功；更换 Blob 节点不改变 BlobId 或会话加密语义。
